...
import { signOut, sendPasswordResetEmail, verifyBeforeUpdateEmail, deleteUser } from 'firebase/auth';
...
export default function SettingsScreen({ navigation }: any) {
  ...
  const handleWebCropAndUpload = async () => {
    ...
    try {
      ...
      window.alert(t('uploadSuccessMessage'));
    } catch (error) {
      console.error(error);
      window.alert(t('uploadErrorMessage'));
    } finally {
      setUploading(false);
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      try {
        if (!auth.currentUser || !userData) throw new Error("Not logged in");
        const manipResult = await ImageManipulator.manipulateAsync(originalUri, [{ resize: { width: 512, height: 512 } }], { compress: 0.7, format: ImageManipulator.SaveFormat.JPEG });
        const formData = new FormData();
        formData.append('userId', auth.currentUser.uid);
        formData.append('username', userData.username);
        // @ts-ignore
        formData.append('avatar', { uri: manipResult.uri, type: 'image/jpeg', name: 'avatar.jpg' });
        const data = await apiClient.postForm('/upload-avatar', formData);
        const downloadUrl = data.url; 
        await updateDoc(doc(db, "users", auth.currentUser.uid), { avatar: downloadUrl });
        setAvatarUrl(downloadUrl);
        Alert.alert(t('uploadSuccessTitle'), t('uploadSuccessMessage'));
      } catch (error) {
        Alert.alert(t('uploadErrorTitle'), t('uploadErrorMessage'));
      } finally {
        setUploading(false);
      }
    }
  ...
  const handleResetPassword = async () => {
    if (auth.currentUser && auth.currentUser.email) {
      try {
        ...
        Platform.OS === 'web' ? window.alert(t('resetPasswordSuccessMessage')) : Alert.alert(t('resetPasswordSuccessTitle'), t('resetPasswordSuccessMessage'));
      } catch (error) {
        Platform.OS === 'web' ? window.alert(t('resetPasswordError')) : Alert.alert(t('uploadErrorTitle'), t('resetPasswordError'));
      }
    }
  };
  ...
  const handleUpdateEmail = async () => {
    ...
    try {
      ...
      Platform.OS === 'web' ? window.alert(t('emailUpdateSentMessage')) : Alert.alert(t('emailUpdateSentTitle'), t('emailUpdateSentMessage'));
  ...
  const handleLogout = () => {
    if (Platform.OS === 'web') {
      if (window.confirm(t('logoutConfirmMessage'))) performLogout();
    } else {
      Alert.alert(t('logoutConfirmTitle'), t('logoutConfirmMessage'), [{ text: t('cancel'), style: "cancel" }, { text: t('logoutButton'), style: "destructive", onPress: performLogout }]);
    }
  };
  ...
  const performAccountDeletion = async () => {
    ...
    try {
      await apiClient.post('/delete-account', { username: userData.username });
      ...
      const msg = error.code === 'auth/requires-recent-login' ? t('reloginRequiredMessage') : t('deleteAccountErrorMessage');
  ...
  const handleDeleteAccount = () => {
    if (Platform.OS === 'web') {
      if (window.confirm(t('deleteAccountConfirmMessage'))) performAccountDeletion();
    } else {
      Alert.alert(t('deleteAccountConfirmTitle'), t('deleteAccountConfirmMessage'), [{ text: t('cancel'), style: "cancel" }, { text: t('deleteAccountConfirmButton'), style: "destructive", onPress: performAccountDeletion }]);
    }
  };
  ...
  const renderThemeSelector = (type: ThemeType, label: string) => {
    ...
    return (
      <TouchableOpacity style={[styles.themeOption, { borderColor: isSelected ? colors.primary : colors.border, backgroundColor: isSelected ? colors.primary + '15' : colors.card }]} onPress={() => setTheme(type)}>
        <Text style={{ color: isSelected ? colors.primary : colors.text, fontWeight: isSelected ? 'bold' : 'normal' }}>{label}</Text>
      </TouchableOpacity>
    );
  };
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} style={{ flex: 1 }}>
        ...
        <TouchableOpacity onPress={pickImage} disabled={uploading} style={styles.avatarContainer}>
          {uploading ? (
            <View style={[styles.placeholder, { backgroundColor: colors.border }]}><ActivityIndicator color={colors.primary} size="large" /></View>
          ) : avatarUrl ? (
            <Image source={{ uri: avatarUrl }} style={[styles.avatar, { borderColor: colors.card }]} />
          ) : (
            <View style={[styles.placeholder, { backgroundColor: colors.border }]}><Text style={styles.placeholderText}>{t('noImage')}</Text></View>
          )}
          <View style={[styles.editIconContainer, { backgroundColor: colors.primary, borderColor: colors.card }]}><Text style={styles.editIcon}>📷</Text></View>
        </TouchableOpacity>
        ...
        <View style={styles.actionsContainer}>
          ...
          {isEditingEmail ? (
            <View style={styles.emailEditWrapper}>
              <TextInput style={[GlobalStyles.input, { marginBottom: 10, backgroundColor: colors.card, color: colors.text, borderColor: colors.border }]} placeholderTextColor={colors.secondaryText} placeholder={t('newEmailPlaceholder')} value={newEmail} onChangeText={setNewEmail} keyboardType="email-address" autoCapitalize="none" />
              <View style={styles.emailEditButtons}>
                <TouchableOpacity onPress={() => setIsEditingEmail(false)} style={[styles.cancelButton, { backgroundColor: colors.border }]}><Text style={[styles.cancelButtonText, { color: colors.text }]}>{t('cancel')}</Text></TouchableOpacity>
                <TouchableOpacity onPress={handleUpdateEmail} style={[styles.saveButton, { backgroundColor: colors.primary }]}><Text style={styles.saveButtonText}>{t('saveButton')}</Text></TouchableOpacity>
              </View>
            </View>
          ) : (
            <TouchableOpacity style={[styles.outlineButton, { borderColor: colors.primary, backgroundColor: colors.card }]} onPress={() => setIsEditingEmail(true)}><Text style={[styles.outlineButtonText, { color: colors.primary }]}>{t('changeEmailButton')}</Text></TouchableOpacity>
          )}
          <TouchableOpacity style={[styles.outlineButton, { borderColor: colors.border, backgroundColor: colors.card, marginTop: 10 }]} onPress={() => navigation.navigate('About')}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}><Ionicons name="information-circle-outline" size={20} color={colors.text} style={{ marginRight: 8 }} /><Text style={[styles.outlineButtonText, { color: colors.text }]}>{t('aboutApp')}</Text></View></TouchableOpacity>
          <TouchableOpacity style={styles.logoutButton} onPress={handleLogout}><Text style={styles.logoutButtonText}>{t('logoutButton')}</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.outlineButton, { borderColor: '#FF3B30', backgroundColor: colors.card, marginTop: 40 }]} onPress={handleDeleteAccount}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}><Ionicons name="trash-outline" size={20} color="#FF3B30" style={{ marginRight: 8 }} /><Text style={[styles.outlineButtonText, { color: '#FF3B30' }]}>{t('deleteAccountButton')}</Text></View></TouchableOpacity>
        </View>
      </ScrollView>
        ...
        <Modal visible={isCropModalVisible} transparent={true}>
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
              <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 15, color: colors.text }}>{t('cropImageTitle')}</Text>
              <View style={styles.cropContainer}><Cropper image={selectedWebImage} crop={crop} zoom={zoom} aspect={1} onCropChange={setCrop} onCropComplete={onCropComplete} onZoomChange={setZoom} /></View>
              <View style={styles.modalButtons}>
                <TouchableOpacity style={[styles.modalButton, { backgroundColor: colors.border, marginRight: 10 }]} onPress={() => setIsCropModalVisible(false)}><Text style={{ color: colors.text, fontWeight: 'bold' }}>{t('cancel')}</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.modalButton, { backgroundColor: colors.primary }]} onPress={handleWebCropAndUpload}><Text style={{ color: '#fff', fontWeight: 'bold' }}>{t('cropAndSave')}</Text></TouchableOpacity>
              </View>
            </View>
          </View>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  profileInfoContainer: { alignItems: 'center', marginBottom: 30, paddingVertical: 15, paddingHorizontal: 30, borderRadius: 15, width: '100%', elevation: 2 },
