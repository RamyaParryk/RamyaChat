...
import { signOut, sendPasswordResetEmail, verifyBeforeUpdateEmail, deleteUser } from 'firebase/auth'; // 🌟 deleteUserを追加
...
// 🌟 アカウント削除処理
  const performAccountDeletion = async () => {
    ...
    try {
      ...
      // 4. 完了メッセージを出してログイン画面へ遷移
      const successMsg = t('deleteAccountSuccessMessage') || "アカウントを削除しました。";
      if (Platform.OS === 'web') {
        window.alert(successMsg);
      } else {
        Alert.alert(t('success') || "完了", successMsg);
      }
      navigation.reset({ index: 0, routes: [{ name: 'Login' }] });
      ...
      if (error.code === 'auth/requires-recent-login') {
        const msg = t('reloginRequiredMessage') || "再ログインが必要です。";
        Platform.OS === 'web' ? window.alert(msg) : Alert.alert(t('error') || "エラー", msg);
      } else {
        // 🌟 エラーメッセージも専用のキーを使用
        const msg = t('deleteAccountErrorMessage') || "削除に失敗しました。";
        Platform.OS === 'web' ? window.alert(msg) : Alert.alert(t('error') || "エラー", msg);
      }
    }
  };
  ...
  const handleDeleteAccount = () => {
    const warningMsg = t('deleteAccountConfirmMessage') || "本当に退会しますか？";
    const title = t('deleteAccountConfirmTitle') || "アカウント削除";
    ...
    if (Platform.OS === 'web') {
      ...
      Alert.alert(
        ...
        [
          ...
          { 
            text: t('deleteAccountConfirmButton') || "削除する", 
            style: "destructive", 
        ...
        <View style={styles.actionsContainer}>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <Text style={[styles.outlineButtonText, { color: '#FF3B30', marginBottom: 0 }]}>
                {t('deleteAccountButton') || 'アカウントを削除する'}
              </Text>
            </View>
          </TouchableOpacity>
